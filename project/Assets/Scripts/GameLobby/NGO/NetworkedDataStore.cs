...
namespace LobbyRelaySample.ngo
{
    ...
    public class NetworkedDataStore : NetworkBehaviour
    {
        ...
        // this can be used to retrieve the host's player data since it isn't synchronized.
        public Dictionary<ulong, PlayerData> AllPlayerData => m_playerData;
        ulong m_localId;
        
        // do I need this?
        public int PlayerCount => m_playerData.Count;
        ...
        public void AddPlayer(ulong id, string name, out PlayerData data)
        {
            if (!IsServer)
            {
                data = null;
            ...
            if (!m_playerData.ContainsKey(id))
                m_playerData.Add(id, new PlayerData(name, id, 0, 0, 0, Role.Innocent, null));
            else
                m_playerData[id] = new PlayerData(name, id, 0, 0, 0, Role.Innocent, null);
        ...
        public int GetPlayerIndex(ulong playerID)
        {
            ...
            {
                IEnumerable<PlayerData> items = m_playerData.Select(kvp => kvp.Value);
                int playerIndex = items.ToList().FindIndex(item => item.id == playerID);
                return playerIndex;
            }
